    ...
    InitExplorerPanel : function() {
        ...
        $("#rightMenu").menu('appendItem', {
        	...
        	onclick: function() {
        		...
        		if(type == 'java') {
                    var singleRunUrl = Global.url.run;
                    var singleRunParam = {
                            extra: "/" + path,
    						mode: "file",
    						appid: appid,
					...
					$.post(singleRunUrl, singleRunParam, function(data) {
						// connect to the websocket to redirect stdout & stdin
						var obj = JSON.parse(data);
						if(obj.code == 0) {
							var wsAddr = 'ws://' + obj.domain + ':' + obj.wsport;
							console.log(wsAddr);
							debugws = new WebSocket(wsAddr);
							debugws.onerror = function(event) {
								consolePanel.append('info', 'SingleFileRun', 'connection error.');
							};
							debugws.onopen = function(event) {
								consolePanel.append('info', 'SingleFileRun', 'connection opened.');
							};
    						debugws.onmessage = function(event) {
    							consolePanel.append('info', 'Output', event.data);
    						};
    						debugws.onclose = function(event) {
    							consolePanel.append('info', 'SingleFileRun', 'connection closed.');
    						};
    					}
    				});
        		} else {
